...
# Texto origal: 00112233445566778899aabbccddeeff
...
#------------------------------------------------------------------------------------------
# Programa principal:
option = input('¿Desea ejecutar el programa en modo manual o estándar? M (Modo manual) | E (Modo estándar)\n')
while ( (option != 'M') and (option != 'E')):
...
if(option != 'E'):
  # Solicitud de clave:
  clave = []
  clave_ = str(input('Clave: '))
  while(len(clave_) != 32):
    print('Clave no válida')
    print('Por favor introduzca un valor de longitud 128 bits en formato hexadecimal')
    clave_ = str(input('Clave: '))
  ...
  # Solicitud de bloque de texto:
  texto_original = []
  texto_original_ = str(input('Bloque de Texto Original: '))
  while(len(texto_original_) != 32):
    print('Bloque de texto no válido')
    print('Por favor introduzca un valor de longitud 128 bits en formato hexadecimal')
    texto_original_ = str(input('Bloque de Texto Original: '))
...
#-------------------------------------------------------------------------------
# Expansión de claves para obtener las roundkey:
...
for n in range(0, 10):
  ...
  for i in range(4):
    columna_aux.append(subclave[n][3][i]) #Cogemos la última columna de la subclave
  ...
  #XOR
  for i in range(4):
    columna1.append(subclave[n][0][i] ^ columna_aux[i] ^ rc[n][i])  # Recuerda que calculas las 4 filas de las columnas
...
#-----------------------------------------------------------------------------------
# Ahora que tenemos las roundkeys podemos aplicar las iteraciones:
